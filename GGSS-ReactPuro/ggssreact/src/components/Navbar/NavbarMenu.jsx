...
import {
  ...
  inputsNumConvenios,
  inputNumDataValores,
  tableValoresHeadings,
  inputNumDataEscala,
  inputDateDataEscala,
  inputNumDataDeducciones,
  inputDateDataDeducciones,
  objectBancos,
  objectEmpresasTelefonia,
  objectSindicatos,
  objectTareas,
...
import {
  addSelectedCargo,
  addSelectedEstado,
  addSelectedEstadoCivil,
  addSelectedEstudio,
  addSelectedFormaPago,
  addSelectedParentesco,
  addSelectedTarea,
  addSelectedTipoDocu,
} from "../../redux/actions/modalesActions";
...
const NavbarMenu = () => {
  ...
  const estudioSelected = useSelector(
    (state) => state.modalState.estudioSelected
  );
  const inputNivelEstudio = useSelector(
    (state) => state.modalState.formulario.inputNivelEstudio
  );
  ...
  const tipoDocumentoSelected = useSelector(
    (state) => state.modalState.tipoDocumentoSelected
  );
  const inputTipoDocumento = useSelector(
    (state) => state.modalState.formulario.inputTipoDocumento
  );
  ...
  //Modos Liquidacion
  const modosLiqValue = useSelector(
  ...
  const estadosValue = useSelector((state) => state.generalState.estados);
  const estadoSelected = useSelector(
    (state) => state.modalState.estadoSelected
  );
  const inputEstado = useSelector(
    (state) => state.modalState.formulario.inputEstado
  );
  ...
  const formaPagoSelected = useSelector(
    (state) => state.modalState.formaPagoSelected
  );
  const inputFormaDePago = useSelector(
    (state) => state.modalState.formulario.inputFormaDePago
  );
  const textAreaFormaPago = useSelector(
    (state) => state.modalState.formulario.textAreaFormaPago
  );
  const valueIdFormaPago = useSelector(
    (state) => state.generalState.idFormaPago
  );

  // cargos
  const cargosValue = useSelector((state) => state.generalState.cargos);
  const cargoSelected = useSelector((state) => state.modalState.cargoSelected);
  const inputCargo = useSelector(
    (state) => state.modalState.formulario.inputCargo
  );
  const textAreaCargo = useSelector(
    (state) => state.modalState.formulario.textAreaCargo
  );
  ...
  const inputTarea = useSelector(
    (state) => state.modalState.formulario.inputTarea
  );
  const textAreaTarea = useSelector(
    (state) => state.modalState.formulario.textAreaTarea
  );
  ...
  const bodyPetEstudio = {
    ...responses.modalDataInputs,
    iDestudios: idEstudio,
  };
  //Tipos de documento
  const idTiposDocumento =
    (tiposDocumentoValue &&
      tiposDocumentoValue[tiposDocumentoValue.length - 1] !== undefined &&
      tiposDocumentoValue[tiposDocumentoValue.length - 1].iDtipoDocumento) + 1;
  const bodyPetTiposDoc = {
    ...responses.modalDataInputs,
  ...
  const provincias =
    provinciasValue &&
    provinciasValue.map((res) => {
      return res.provincia;
    });

  const localidades =
    localidadesValue &&
    localidadesValue.map((res) => {
      return res.localidad;
    });

  const barrios =
    barriosValue &&
    barriosValue.map((res) => {
      return res.barrio;
    });

  const cargosMap =
    cargosValue &&
    cargosValue.map((cargo, i) => {
      return cargo.nombreCargo;
    });

  const tareasMap =
    tareaValues &&
    tareaValues.map((tarea, i) => {
      return tarea.tareaDesempeñada;
    });

  const parentescosMap =
    parentescosValue &&
    parentescosValue.map((parent, i) => {
      return parent.nombreParentesco;
    });

  const formasDePagoMap =
    formasDePagoValue &&
    formasDePagoValue.map((forma, i) => {
      return forma.nombreFormadePago;
    });

  const modosContratacionMap =
    modosContratacionValue &&
    modosContratacionValue.map((modo, i) => {
      return modo.modoContratacion;
    });

  const modosLiquidacionMap =
    modosLiqValue &&
    modosLiqValue.map((modo, i) => {
      return modo.modoLiquidacion;
    });

  const empleadoresMap =
    empleadoresValue &&
    empleadoresValue.map((empl, i) => {
      return empl.razonSocial;
    });
          ...
          <div className="collapse navbar-collapse" id="navbarNav">
            <ul className=" navbar-nav">
                    ...
                    <ul class="dropdown-menu">
                      ...
                      >
                        ...
                        <ButtonCallModal
                          ...
                          nameButton="Estados para empleados"
                          useNavbar={true}
                        />
                        <ButtonCallModal
                          idModal="cargos"
                        ...
                        <ButtonCallModal
                          ...
                          nameButton="Formas de Pago"
                          useNavbar={true}
                        />
                        <ButtonCallModal
                          idModal="modosDeContratacion"
                          nameButton="Modos de Contratación"
                          useNavbar={true}
                        />
                        <ButtonCallModal
                        ...
                        />
                        <ButtonCallModal
                          idModal="motivosEgreso"
                          nameButton="Motivos de Egreso"
                          useNavbar={true}
                        />
                        <hr />
                        <ButtonCallModal
                        ...
                        <ButtonCallModal
                          idModal="pdlb"
                          nameButton="Provincias - Departamentos - Localidades - Barrios"
                          useNavbar={true}
                        />
                        <ButtonCallModal
                          idModal="calles"
                        ...
                        <ButtonCallModal
                          idModal="empleadores"
                          nameButton="Empleadores"
                          useNavbar={true}
                        />
                        <ButtonCallModal
                          idModal="alicuotas"
                          nameButton="Alicuotas"
                          useNavbar={true}
                        />
                      </div>
                    </ul>
                  ...
                  <li class="dropdown-submenu">
                    ...
                    <ul class="dropdown-menu">
                      ...
                      >
                        ...
                        <ButtonCallModal
                          idModal="Telefonia"
                          nameButton="Empresas de telefonia celular"
                          useNavbar={true}
                        />
                        <hr />
                        <ButtonCallModal
                          idModal="Sindicatos"
                          nameButton="Sindicatos"
                          useNavbar={true}
                        />
                        <ButtonCallModal
                          idModal="ObrasSociales"
                          nameButton="Obras Sociales"
                          useNavbar={true}
                        />
                        <ButtonCallModal
                          idModal="AFJP"
                        ...
                        <hr />
                        <ButtonCallModal
                          idModal="CentrosCosto"
                          nameButton="Centros de Costo"
                          useNavbar={true}
                        />
                        <ButtonCallModal
                          idModal="SectoresDeptos"
                          nameButton="Sectores/Departamentos"
                          useNavbar={true}
                        />
                        <ButtonCallModal
                          idModal="Direcciones"
                          nameButton="Direcciones"
                          useNavbar={true}
                        />
                        <ButtonCallModal
                          idModal="LugaresPago"
                          nameButton="Lugares de Pago"
                          useNavbar={true}
                        />
                        <hr />
                        <ButtonCallModal
                          idModal="Documentacion"
                          nameButton="Documentación que presentan los empleados"
                          useNavbar={true}
                        />
                        <hr />
                        <ButtonCallModal
                          idModal="Reduccion"
                          nameButton="Reducción de Deducciones de Ganancias"
                          useNavbar={true}
                        />
                        <ButtonCallModal
                          idModal="Escala"
                          nameButton="Escala de Ganancias"
                          useNavbar={true}
                        />
                        <ButtonCallModal
                          idModal="Deducciones"
                          nameButton="Deducciones de Ganancias"
                          useNavbar={true}
                        />
                        <hr />
                        <ButtonCallModal
                          idModal="Valores"
                          nameButton="Valores Permanencia Categoría"
                          useNavbar={true}
                        />
                        <hr />
                        <ButtonCallModal
                          idModal="Convenios"
                          nameButton="Convenios, Categorías, Básicos y Antigüedad"
                          useNavbar={true}
                        />
                        <ButtonCallModal
                          idModal="Jerarquia"
                          nameButton="Jerarquía de Categorías"
                          useNavbar={true}
                        />
                        <ButtonCallModal
                          idModal="Licencias"
                          nameButton="Licencias por Antigüedad"
                          useNavbar={true}
                        />
                        <hr />
                        <ButtonCallModal
                          idModal="Plan"
                          nameButton="Plan de Cuentas"
                          useNavbar={true}
                        />
                      </div>
                    </ul>
              ...
              <BasicModal
                ...
                firstOptionCompare={
                  inputMascEstadosCiviles
                    ? inputMascEstadosCiviles
                    : estadoCivilSelected.masculino
                }
                secondOptionCompare={
                  inputFemEstadosCiviles
                    ? inputFemEstadosCiviles
                    : estadoCivilSelected.femenino
                }
              ...
              <BasicModal
                ...
                firstOptionCompare={
                  inputParentesco
                    ? inputParentesco
                    : parentescoSelected.nombreParentesco
                }
                secondOptionCompare={
                  inputParentesco
                    ? inputParentesco
                ...
                valueObs={
                  textAreaParent ? textAreaParent : parentescoSelected.obs
                }
                valueCheckbox={
                  inputAsignacionParent
                    ? inputAsignacionParent
                    : parentescoSelected.generaAsignacion
                }
                valueCheckboxNum={
                  inputGananciaParent
                    ? inputGananciaParent
                    : parentescoSelected.deduceGanancias
                }
                valueNumCheck={
                  inputImporteParent
                    ? inputImporteParent
                    : parentescoSelected.importeDeduce
                }
              ...
              <BasicModal
                ...
                firstOptionCompare={
                  inputEstado ? inputEstado : estadoSelected.nombreEstado
                }
                secondOptionCompare={
                  inputEstado ? inputEstado : estadoSelected.nombreEstado
                }
              ...
              <BasicModal
                ...
                firstOptionCompare={
                  inputFormaDePago
                    ? inputFormaDePago
                    : formaPagoSelected.nombreFormadePago
                }
                secondOptionCompare={
                  inputFormaDePago
                    ? inputFormaDePago
                ...
                valueObs={
                  textAreaFormaPago ? textAreaFormaPago : formaPagoSelected.obs
                }
              ...
              <BasicModal
                ...
                firstOptionCompare={
                  inputCargo ? inputCargo : cargoSelected.nombreCargo
                }
                secondOptionCompare={
                  inputCargo ? inputCargo : cargoSelected.nombreCargo
                }
                valueObs={
                  textAreaCargo ? textAreaCargo : cargoSelected.observacion
                }
              ...
              <BasicModal
                ...
                firstOptionCompare={
                  inputTarea ? inputTarea : tareaSelected.tareaDesempeñada
                }
                secondOptionCompare={
                  inputTarea ? inputTarea : tareaSelected.tareaDesempeñada
                }
              ...
              <BasicModal
                idModal="motivosEgreso"
                nameModal="Motivos de Egreso"
                placeholder={objectMotivosEgreso}
                textArea={true}
              />
              <BasicModal
                idModal="paises"
                nameModal="Paises"
                placeholder={objectPaises}
              />
              <BasicModal
                idModal="nacionalidades"
                nameModal="Nacionalidades"
                placeholder={objectPaises}
              />
              <ModalPDLB
                idModal="pdlb"
              ...
              <BasicModal
                idModal="alicuotas"
                nameModal="Alicuotas"
                placeholder={objectAlicuotas}
                inputNum={true}
                inputNumName="Alicuota"
                hasCheckbox={true}
                checkboxName="Pide N° CUIT"
              />
              ...
              <BasicModal
                idModal="Telefonia"
                nameModal="Empresas de Telefonia"
                placeholder={objectEmpresasTelefonia}
              />
              <BasicModal
                idModal="Sindicatos"
                nameModal="Sindicatos"
                placeholder={objectSindicatos}
                dropdown={true}
              />
              <BasicModal
                idModal="ObrasSociales"
                nameModal="Obras Sociales"
                placeholder={objectObrasSociales}
                inputNum={true}
                inputNumName="Porcentaje Patronal"
                textArea={true}
              />
              <BasicModal
                idModal="AFJP"
                nameModal="A.F.J.P"
                placeholder={objectAFJP}
                inputNum={true}
                inputNumName="Porcentaje Patronal"
                textArea={true}
              />
              <BasicModal
                idModal="CentrosCosto"
                nameModal="Centros de Costo"
                placeholder={objectCentrosCosto}
                dropdown={true}
                textArea={true}
              />
              <BasicModal
                idModal="SectoresDeptos"
                nameModal="Sectores/Departamentos"
                placeholder={objectSectoresDptos}
                dropdown={true}
                textArea={true}
              />
              <BasicModal
                idModal="Direcciones"
                nameModal="Direcciones"
                placeholder={objectDirecciones}
                textArea={true}
                relacion={true}
                nameRelacion="Sector/Dpto"
              />
              <BasicModal
                idModal="LugaresPago"
                nameModal="Lugares de Pago"
                placeholder={objectLugaresPago}
                textArea={true}
              />
              <BasicModal
                idModal="Documentacion"
                nameModal="Documentación"
                placeholder={objectDocumentacion}
                textArea={true}
              />
              <ModalTable
                idModal="Reduccion"
                nameModal="Tabla de Reducción de Deducciones"
                column={tableReduccionHeadings}
                btnAceptar={true}
              />
              <ModalEscala
                idModal="Escala"
                nameModal="Escala de Ganancias"
                inputNumData={inputNumDataEscala}
                hasInputDate={true}
                inputDateData={inputDateDataEscala}
                table={true}
                buttonNum={true}
                flex={true}
                styleContainer={{ height: "600px", width: "auto" }}
                styleData={{ height: "350px" }}
              />
              <ModalEscala
                idModal="Deducciones"
                nameModal="Deducciones de Ganancias"
                inputNumData={inputNumDataDeducciones}
                hasInputDate={true}
                inputDateData={inputDateDataDeducciones}
                styleContainer={{ height: "400px", width: "auto" }}
                styleData={{ height: "350px" }}
              />
              <ModalEscala
                idModal="Valores"
                nameModal="Valores Permanencia Categoría"
                inputNumData={inputNumDataValores}
                tableValores={true}
                column={tableValoresHeadings}
                flex={true}
                categorias={true}
                buttonNumTable={true}
                styleContainer={{ height: "430px", width: "auto" }}
                styleData={{ height: "350px" }}
              />
              <ModalConvenios
                idModal="Convenios"
                nameModal="Convenios, Categorías, Básicos y Antigüedad"
                placeholder={objectConvenios}
                inputsNumConvenios={inputsNumConvenios}
                column={tableConvenios}
                placeholderCategorias={objectCategorias}
                inputsNumCategorias={inputsNumCategorias}
              />
              <ModalTable
                idModal="Jerarquia"
                nameModal="Jerarquía de las Categorías"
                column={tableJerarquia}
                dropdown={true}
                jerarquia={true}
              />
              <ModalTable
                idModal="Licencias"
                nameModal="Licencias por Antigüedad"
                licencias={true}
                column={tableLicencias}
                objectInputs={inputsNumLicencias}
              />
