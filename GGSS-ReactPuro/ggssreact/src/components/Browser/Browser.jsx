      ...
      useEffect(()=>{
        ...
        .then(res => {
            ...
            if(empData.apellido.length > 0){
                getEmployeByName(data, empData.apellido).then(res => setListEmpleados(res))
                return;
            }
            if(empData.legajo.length > 0){
                getEmployeByLegajo(data, empData.legajo).then(res => setListEmpleados(res))
                return;
            }
  ...
  return (
    <div className='Lateral-Izquierdo'>
        ...
        <div className='row mt-1 mr-2'>
                    <input onChange={(e)=> onInputChange(e)} value={empData.legajo} className='form__grupo__input__browser mr-2' type="text" name="legajo" id="legajos" placeholder='Ingrese Legajo '/>
                </div>
                <div className='row mt-1 mr-2'>
                    <input onChange={(e)=> onInputChange(e)} value={empData.apellido} className='form__grupo__input__browser mr-2' type="text" name="apellido" id="nombres" placeholder='Ingrese Nombre '/>
                </div>
            <select className="form-select row mt-1 selectMenu" multiple aria-label="multiple select example">
            {
                listEmpleados.map(emp=>{
                    return(
                        <option onClick={(e)=> onSelect(e, (emp.apellido), (emp.iDempleado))} value="1">{`${emp.apellido}, ${emp.nombres}`}</option>
                    )
                })
            }
        </select>
